...
def gen_seed_func(all_factors):
	...
	sub hidden layer nodes
	for layers add a bias
	#maybe add something to change ativation function
	'''

	possiblities = [awdsf, awdsf, 1]
...
for comp in all_nn_ready():
	...
	pdata.to_csv("test_"+company["name"]+".csv")
	...
	callb = [
		keras.callbacks.ModelCheckpoint("./models/"+company["name"].replace(" ", "_")+("/{epoch:02d}-{mean_squared_error:.2f}.keras"), monitor="mean_squared_error")#batch was removed for now
	]
	...
	#what?????????????	
	model.fit(x=inputdata, y=outputdata, validation_split=0.1, epochs=100, callbacks=callb, verbose=0)#maybe config batchsize
	...
	for i in range(5):
		...
		for mod in models:
			...
			results.append(model.history)
'''
- add support for competitors in dict_to_arr and google trends
- testing against the next day's open, should be index 0 in raw csv
-structure: inp-relu-relu-linear
'''
#testing linear o, relu hidden - 5K mse
#testing linear o, sigmoid hidden - 720 mse
#testing linear o, relu hidden double layer - 4k mse
#testing linear o, sigmoid hidden double layer - 20k mse
